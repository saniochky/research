...
class GetData:
    ...
    def __init__(self, country=None, index=None, year=None):
        ...
        if country and index and year:
            ...
            self._iso = ISO[self._country]
            self._index = INDEX_DICT[index]
            self._year = year
            self._api_key = API_KEY
            self._data = self.get_quandl()
    ...
    def get_quandl(self):
        ...
        value = None
        url = f'https://www.quandl.com/api/v3/datasets/WESV/{self._iso}_{self._index}.json?api_key={self._api_key}'
